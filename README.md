...
线上预览：[https://whitelunatix.dpdns.org/](https://whitelunatix.dpdns.org/)

首次使用请查看：[详细使用说明](docs/user-guide.md)
...
| `ADMIN_TOKEN` / `USER_TOKEN` | 仅用于兼容旧版共享令牌，新部署请使用网页账户系统。 |
| `DB_PATH` | 可选数据库路径，默认使用 `data/stores.db`。 |

首次管理员只能在服务所在电脑上创建。完成初始化后，可在“设置 → 用户管理”中创建管理员、操作员或访客账户；向局域网或公网开放前请先完成初始化。

账户角色：

- `管理员`：管理用户、系统配置、完整备份和所有日常操作。
- `操作员`：管理店铺、刷新数据、调整分类，不可管理用户或导入完整备份。
- `访客`：浏览商品、价格和历史数据，个人隐藏店铺及曲线偏好独立保存。

登录会话默认有效 7 天。停用用户或重置密码会立即注销该用户已有会话，成功的写操作会记录到审计日志。
